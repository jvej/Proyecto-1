...
public class ReservaDAO {
    ...
    public List<Reserva> listarActivasPorFecha(LocalDate fecha) {
        List<Reserva> resultado = new ArrayList<>();
        for (Reserva r : listar()) {
            if (r.getFecha().equals(fecha) && "ACTIVA".equals(r.getEstado())) resultado.add(r);
        }
        return resultado;
    }
    ...
    public String generarNuevoId() {
        ...
        for (Reserva r : listar()) {
            try {
                max = Math.max(max, Integer.parseInt(r.getId().replace("RES-", "")));
    ...
    private Reserva convertirAObjeto(Element el) {
        ...
        r.setFuncionarioId(XMLManager.getTexto(el, "idFuncionario"));   // <- tag de Coso
        ...
        r.setRecursosIds(leerListaPlana(el, "recurso"));                // <- tag plano de Coso
        return r;
    }
    ...
    private Element convertirAElemento(Document doc, Reserva r) {
        ...
        XMLManager.crearHijoTexto(doc, el, "idFuncionario", r.getFuncionarioId());
        ...
        // Jeferson espera <recurso> repetido DIRECTO bajo <reserva>, sin contenedor.
        for (String recursoId : r.getRecursosIds()) {
            XMLManager.crearHijoTexto(doc, el, "recurso", recursoId);
        }
    ...
    // Igual que leerLista, pero se usa cuando el tag está plano (sin contenedor),
    // directo bajo <reserva> (como pide Coso para "recurso").
    private List<String> leerListaPlana(Element padre, String tagHijo) {
        List<String> lista = new ArrayList<>();
        NodeList nodos = padre.getElementsByTagName(tagHijo);
        for (int i = 0; i < nodos.getLength(); i++) lista.add(nodos.item(i).getTextContent());
        return lista;
